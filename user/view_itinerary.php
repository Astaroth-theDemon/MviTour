...
$stmt = $conn->prepare("SELECT * FROM SavedItineraries WHERE itinerary_id = ? AND user_id = ?");
...
// After getting the saved itinerary data and decoding it
$saved_itinerary = $result->fetch_assoc();
$_POST = json_decode($saved_itinerary['itinerary_data'], true);
...
$accommodation_types = isset($_POST['accommodation']) ? explode(",", $_POST['accommodation']) : [];
...
function buildOptimizedDailyItinerary($spots, $restaurants, $accommodation, $includeDining, &$usedRestaurants) {
    ...
    foreach ($timeSlots as $timeSlot) {
        ...
        if ($includeDining && !empty($restaurants)) {
            $spotLocation = new Location(
                $currentSpot['tourist_spot_id'] ?? $currentSpot['business_id'],
                'spot',
...
$tourist_query = $conn->prepare("
    SELECT *, latitude, longitude FROM tourist_spots
    WHERE location = ? AND entrance_fee <= ? AND category IN (" . implode(",", array_fill(0, count($sights), "?")) . ")
    AND status = 'active'
    AND is_open = 1
");
...
if ($include_dining) {
    $restaurant_query = $conn->prepare("
        SELECT *, latitude, longitude FROM businesses
        WHERE location = ? 
        AND budget <= ? 
...
if ($accommodation_needed && !empty($accommodation_types)) {
    ...
    $accommodation_type = is_array($accommodation_types) ? $accommodation_types[0] : $accommodation_types;
...
// Use the same optimization function as in itinerary_results.php
...
function calculateTotalCost($duration, $people, $tourist_spots, $restaurants, $accommodation = null) {
    ...
    // Restaurant costs (if dining is included)
    if (!empty($restaurants)) {
        // Calculate average meal cost
        $total_restaurant_cost = array_sum(array_column($restaurants, 'budget'));
        $avg_meal_cost = count($restaurants) > 0 ? $total_restaurant_cost / count($restaurants) : 0;
        
        // Assume 3 meals per day
        $meals_per_day = 3;
        $total_cost += ($avg_meal_cost * $meals_per_day * $duration * $people);
    }
...
// Prepare error messages for missing or insufficient options
$error_messages = [];
$restaurant_error = '';

if (empty($all_tourist_spots)) {
    $error_messages[] = "No tourist spots found matching your criteria in $destination.";
}

if ($include_dining && empty($restaurants)) {
    $restaurant_error = "No restaurants found within your budget in $destination.";
    $error_messages[] = $restaurant_error;
}

if ($accommodation_needed && !$recommended_accommodation) {
    $error_messages[] = "No accommodation found matching your criteria in $destination.";
}
?>
    ...
    <style>
        * {
        ...
        .download-btn-container {
            position: fixed;
            top: 110px; /* Positioned below modify button (120px + some spacing) */
                    ...
                    <div class="timeline-item">
                        ...
                        class="timeline-content <?php echo $contentClass; ?>">
                            <div class="timeline-time">
                                <?php echo ucfirst(str_replace('_dining', ' Dining', $item['time'])); ?>
                            </div>
                            ...
                            <?php /* Restaurant price information removed as requested */ ?>
                        </a>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('downloadBtn').addEventListener('click', async function() {
                ...
                try {
                    ...
                    const elements = {
                        badges: container.querySelectorAll('.category-badge'),
                        times: container.querySelectorAll('.timeline-time'),
                        diningInfo: container.querySelectorAll('.dining-info'),
                        timelineDots: container.querySelectorAll('.timeline-dot')
                    };

                    // Apply enhanced styles for capture
                    enhanceElementsForCapture(elements);

                    // Wait for styles to apply
                    await new Promise(resolve => setTimeout(resolve, 500));

                    // Store original styles
                    const originalStyles = storeOriginalStyles(container);

                    // Set capture styles
                    setCaptureStyles(container);
            ...
            // Helper Functions
            function enhanceElementsForCapture(elements) {
                // Enhance category badges
                elements.badges.forEach(badge => {
                    enhanceBadgeStyles(badge);
                });

                // Enhance timeline times
                elements.times.forEach(time => {
                    enhanceTimeStyles(time);
                });

                // Enhance dining info boxes
                elements.diningInfo.forEach(info => {
                    enhanceDiningInfoStyles(info);
                });

                // Enhance timeline dots
                elements.timelineDots.forEach(dot => {
                    enhanceDotStyles(dot);
                });
            }

            function enhanceBadgeStyles(badge) {
                const styles = {
                    display: 'inline-flex',
                    visibility: 'visible',
                    alignItems: 'center',
                ...
                if (span) {
                    enhanceTextElement(span);
                }

                // Style badge icon
                const icon = badge.querySelector('i');
                if (icon) {
                    enhanceIconElement(icon);
                }
            }

            function enhanceTimeStyles(time) {
                time.style.setProperty('display', 'inline-block', 'important');
                time.style.setProperty('visibility', 'visible', 'important');
                time.style.setProperty('opacity', '1', 'important');
            }

            function enhanceDiningInfoStyles(info) {
            ...
            function enhanceIconElement(element) {
                const styles = {
                    display: 'inline-block',
                    visibility: 'visible',
                    color: 'white',
            ...
            function storeOriginalStyles(container) {
                return {
                    padding: container.style.padding,
                    timeline: document.querySelector('.timeline').style.marginBottom
                };
            }

            function setCaptureStyles(container) {
                container.style.padding = '30px';
                document.querySelector('.timeline').style.marginBottom = '150px';
            }

            function restoreOriginalStyles(container, originalStyles) {
                container.style.padding = originalStyles.padding;
                document.querySelector('.timeline').style.marginBottom = originalStyles.timeline;
            }

            function enhanceClonedDocument(clonedDoc) {
                const elements = {
                    badges: clonedDoc.querySelectorAll('.category-badge'),
                    times: clonedDoc.querySelectorAll('.timeline-time'),
                    diningInfo: clonedDoc.querySelectorAll('.dining-info'),
                    timelineDots: clonedDoc.querySelectorAll('.timeline-dot')
                };
